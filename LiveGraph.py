#Make a live Graph simulate temperature and humidity by random numbers.
...
#    hum, temp = Adafruit_DHT.read_retry(11, 17)
    temp = np.random.random()*sin(2.5*pi*cnt)*10+20
    hum = np.random.random()*2+50
    ...
    cnt = cnt + 0.1
    if (cnt > 5):  # If you have 50 or more points, delete the first one from the array
